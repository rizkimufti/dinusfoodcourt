...
$pdf->addText(220, 820, 16,'<b>Laporan Pemesanan</b>');
...
$sql = mysql_query("SELECT hubungi.No Pemesanan as faktur,DATE_FORMAT(Tanggal, '%d-%m-%Y') as tanggal,
                    Nama Barang,jumlah,Harga 
                    FROM hubungi,produk  
                    WHERE (hubungi.No Pemesanan=produk.No Pemesanan) 
                    AND (hubungi.No Pemesanan=hubungi.No Pemesanan)
                    AND (hubungi.status='Lunas') 
                    AND (hubungi.Tanggal='$sekarang')
					AND (hubungi.Tanggal BETWEEN '$mulai' AND '$selesai;
